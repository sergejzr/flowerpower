...
public class TopicFlowerCreatorWithLemma {
	...
	private File cleaneddir;
	private File statisticsfile;
	private boolean usestatistics;

	public TopicFlowerCreatorWithLemma() {
		...
		options.addOption(Option.builder().longOpt("statisticsoutput").hasArg()
				.desc("A file where statistics about the input data should be stored.")
				.required(false).build());
		
		options.addOption(Option.builder().longOpt("usestatistics").hasArg()
				.desc("If --usestatistics is set, the file will be re-used")
				.required(false).build());
	...
	private void readOptions(CommandLine cmd) throws ParseException {
		...
		options.addOption(Option.builder().longOpt("statisticsfile").hasArg()
				.desc("A file where statistics about the input data should be stored.")
				.required(false).build());
		

		
		String statisticsfilestr = cmd.getOptionValue("statisticsfile", null);
		if (statisticsfilestr != null) {
			statisticsfile = new File(statisticsfilestr);

			if (statisticsfile.isDirectory()) {
				throw new ParseException("The optional argument 'ldamodelfile' shoud be a file name");
			}
			if (!statisticsfile.getAbsoluteFile().getParentFile().exists()) {
				throw new ParseException("WIll not be able to write the model, directory does not exist: "
						+ statisticsfile.getAbsoluteFile().getParentFile());
		...
		usestatistics = cmd.hasOption("usestatistics");
	...
	public static void main(String[] args) {
		...
		//
		TopicFlowerCreatorWithLemma tf = new TopicFlowerCreatorWithLemma();
