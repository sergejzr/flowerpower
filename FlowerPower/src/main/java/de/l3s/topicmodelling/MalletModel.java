...
public class MalletModel {
	...
	private void testArtificialDocument(int testid) {
		...
		for (int i = 0; i < 3; i++)
			System.out.println(stopics.get(i).getTopicid() + "\t" + stopics.get(i).getScore() + " should be "
					+ ind.get(testid).score + lable(stopics.get(i).getTopicid()));

	}

	private Map<Integer, ScoredTopics> indexScoredTopics(List<ScoredTopics> stopics) {
	...
	/*
	 * private double[] inferTopicsFor(String str) { InstanceList testing = new
	 * InstanceList(instances.getPipe());
	 * 
	 * testing.addThruPipe(new Instance(str, null, "test instance", null));
	 * 
	 * TopicInferencer inferencer = model.getInferencer(); double[]
	 * testProbabilities = inferencer.getSampledDistribution(testing.get(0), 10,
	 * 1, 5); return testProbabilities; }
	 */
	...
	public void addTrainingInstance(String str) throws UnsupportedEncodingException {
		...
		instances.addThruPipe(new CsvIterator(fileReader, pattern, 3, 2, 1)); // data,
																				// label,
																				// name
																				// fields}
	}
