...
public class MICategoryCal2 {
	...
	public MICategoryCal2() {
		// TODO Auto-generated constructor stub
	}
	public MICategoryCal2( HashMap<Integer, ArrayList<String>> CategoryDocumentsMap,ParallelTopicModel model) {
	...
	private void computeValues(Integer category) {
        ...
        double[] distribution_avg = new double[200];
        for(int i=0; i<200;i++)
        	distribution_avg[i]=0;
        int count=0;
		/*for(String doc:CategoryDocumentsMap.get(category))//need to get the actual documents
		{
		 ...
		 for(int i=0; i<200;i++)
	        	distribution_avg[i]=distribution_sum[i]/count;*/
		 for(int i=0; i<200;i++)
		 {
			 if(TopicFrequencyMap.get("t"+i)==null)
				 continue;
			 distribution_avg[i]=(double)TopicFrequencyMap.get("t"+i)/numberOfWordsInCategory;
		 }
		 ...
		 //MI(z_i,c)
		 ArrayList<TermMI> list= new ArrayList<MICategoryCal2.TermMI>();
		 for(int i=0; i<200;i++)
		 {
			 double score=distribution_avg[i]*(Math.log(distribution_avg[i]/(topicDistribution[i]*prob_of_category)));
