...
public class MIselection2 {
	 ...
	 */
	public MIselection2(ArrayList aPositiveTrainList,
	...
	public static void test1() {
		...
		try {
			...
			MIselection2 mi = new MIselection2(positiveList, negativeList);
	...
	public static Hashtable<listtype, Collection<String>> selectFeatures(
		...
		Hashtable<listtype, ArrayList<Terms>> lists = MIselection2.getTagLists(
				number, con, sql);
			...
			ArrayList<TreeSet<IDSorter>> topicSortedWords = model.getSortedWords();
		       ...
		       for(int i=0; i<model.getData().size();i++)
		       {
		    	   int winneridx=0;
			        ...
			        double t4 = distribution[indlist[3]];
			        double t5 = distribution[indlist[4]];
			        ...
			        for(int k=0; k<t4*100; k++)
			        {
			        	str+="t"+indlist[3]+" ";
			        }
			        for(int k=0; k<t5*100; k++)
			        {
			        	str+="t"+indlist[4]+" ";
			        }
			        str.trim();
			        if(topicmap.get(indlist[0])==null)
			        {
			        	ArrayList<String> temparray= new ArrayList<String>();
			        	temparray.add(str);
			        	topicmap.put(indlist[0], temparray);
			        }
			        else
			        {
			        	ArrayList<String> temparray = topicmap.get(indlist[0]);
			        	temparray.add(str);
			        	topicmap.remove(indlist[0]);
			        	topicmap.put(indlist[0], temparray);
			        }
		       ...
		       for(int posTopic=0; posTopic < numtopics; posTopic++)
		       {
					...
					*/
					/*WordTopicScoreTriple[] wtss= new WordTopicScoreTriple[mi.posResultList.size()/3];
					int n=topicmap.get(posTopic).size();
					Iterator itera = mi.posResultList.iterator();
					int i=0;
					while(itera.hasNext())
					{
						if(i>= mi.posResultList.size()/3)
							break;
						TermMI mm=(TermMI)itera.next();
						String str = mm.term;
						WordTopicScoreTriple w= new WordTopicScoreTriple();
						w.setTopic(posTopic);
						//we cant find all the terms in the map since we only considered the top 2000
						if(map.get(str+posTopic)==null)
							continue;
						double t = (map.get(str+posTopic));
						w.setScore(t*(mm.value));
						w.setWord(str);
						wtss[i]=w;
						i++;
					}
					...
					for(WordTopicScoreTriple w: wtss)
					{
